...
async function performRegistration(page: Page): Promise<string> {
    ...
    await registrationPage.settele(RandomDataUtil.getPhoneNumber());

    await registrationPage.setpwd("test123");
    await registrationPage.setCpwd("test123");

    await registrationPage.setcheck();  // Accept the privacy policy
